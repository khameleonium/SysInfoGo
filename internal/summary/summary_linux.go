...
func collect(ctx context.Context) (*Info, []output.Warning, error) {
	...
	if info.Virtualization == "" {
		if _, err := os.Stat("/.dockerenv"); err == nil {
			info.Virtualization = "Docker (guest)"
		} else if strings.Contains(strings.ToLower(info.Kernel), "microsoft") || strings.Contains(strings.ToLower(info.Kernel), "wsl") {
			info.Virtualization = "WSL (guest)"
		}
	}
